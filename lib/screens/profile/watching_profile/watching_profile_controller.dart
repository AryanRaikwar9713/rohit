...
class WatchingProfileController extends GetxController {
    ...
    await getProfileFuture(
      CoreServiceApis().getWatchingProfileList(
        ...
        .then((v) {
          ...
          // Auto-select profile and navigate to dashboard if navigateToDashboard is true
          if (navigateToDashboard && accountProfiles.isNotEmpty) {
            WatchingProfileModel? profileToSelect;

            // First, try to select saved profile if it exists
            if (profileId.value > 0 &&
                accountProfiles
                    .any((element) => element.id == profileId.value)) {
              profileToSelect = accountProfiles
                  .firstWhere((element) => element.id == profileId.value);
            } else {
              // Try to find a profile without PIN protection first (for smoother auto-navigation)
              profileToSelect = accountProfiles.firstWhere(
                (element) => !(element.isProtectedProfile.getBoolInt() &&
                    element.profilePin.isNotEmpty),
                orElse: () => accountProfiles
                    .first, // Fallback to first profile if all have PIN
              );
            }

            // Auto-select and navigate if profile is found
  ...
  Future<void> logoutCurrentUser() async {
    ...
    await AuthServiceApis()
        .deviceLogoutApi(deviceId: yourDevice.value.deviceId)
        .then((value) async {
      isLoggedIn(false);
      AuthServiceApis().removeCacheData();
